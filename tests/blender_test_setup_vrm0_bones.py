...
def test_right_toes_direction_is_same_as_foot(context: Context) -> None:
    clean_scene(context)
    ops.icyp.make_basic_armature()
    armature = context.view_layer.objects.active
    if not armature or not isinstance(armature.data, Armature):
...
def test_no_left_toes(context: Context) -> None:
    clean_scene(context)
    ops.icyp.make_basic_armature()
    armature = context.view_layer.objects.active
    if not armature or not isinstance(armature.data, Armature):
    ...
    right_foot_leg = armature.data.edit_bones["foot.R"]
    right_foot_leg.tail = Vector((-0.2, 0.2, 0))
    right_foot_leg.tail = Vector((-0.2, -0.2, 0))
    bpy.ops.object.mode_set(mode="OBJECT")
    # return
    setup_bones(context, armature)
...
def test_no_right_fingers(context: Context) -> None:
    clean_scene(context)
    ops.icyp.make_basic_armature()
    armature = context.view_layer.objects.active
    if not armature or not isinstance(armature.data, Armature):
...
def test_left_finger_only_index_index_intermediate(context: Context) -> None:
    clean_scene(context)
    ops.icyp.make_basic_armature()
    armature = context.view_layer.objects.active
    if not armature or not isinstance(armature.data, Armature):
...
FUNCTIONS = [
    ...
    test_right_toes_direction_is_same_as_foot,
    test_no_left_toes,
    test_no_right_fingers,
    test_left_finger_only_index_index_intermediate,
]
