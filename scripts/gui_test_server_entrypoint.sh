...
(set +e; (while true; do Xvfb :0 -screen 0 1600x900x24 -fbdir "$(mktemp -d)"; done) 2>&1 | tee var/log/Xvfb.log) &
...
(set +e; (while true; do x11vnc -display "$DISPLAY" -forever; done) 2>&1 | tee var/log/x11vnc.log) &
