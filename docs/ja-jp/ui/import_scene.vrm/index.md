...
VRMファイルの配置場所と同じ場所にフォルダを作成し、VRMファイル内に存在するテクスチャを展開します。チェックをしない場合、テクスチャは現在編集中の
`.blend` ファイル内に埋め込まれます。
...
VRMファイルの読み込み後、3Dビューポートのシェーディング設定をマテリアルプレビューに設定します。こちら、Blenderに慣れている人にとってはあまり意味のない設定ではありますが、まだ慣れていない人が
インポート直後に、別プラットフォームでの表示とほぼ同じ表示を再現することによって、テンションを上げて欲しいという思いがあります。
