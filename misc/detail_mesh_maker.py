...
class ICYP_OT_DETAIL_MESH_MAKER(bpy.types.Operator):  # noqa: N801
    ...
    face_center_ratio: bpy.props.FloatProperty(
        default=1, min=0.2, max=1, soft_min=0.6, name="Face center ratio"  # noqa: F722
    )
    eye_width_ratio: bpy.props.FloatProperty(default=2, min=0.5, max=4, name="Eye width ratio")  # noqa: F722
    nose_head_height: bpy.props.FloatProperty(default=1, min=0, max=1, name="nose head")  # noqa: F722
    nose_top_pos: bpy.props.FloatProperty(default=0.2, min=0, max=0.6, name="nose top position")  # noqa: F722
    nose_height: bpy.props.FloatProperty(default=0.015, min=0.01, max=0.1, step=0.001, name="nose height")  # noqa: F722
    nose_width: bpy.props.FloatProperty(default=0.5, min=0.01, max=1, name="nose width")  # noqa: F722
    eye_depth: bpy.props.FloatProperty(default=0.01, min=0.01, max=0.1, name="Eye depth")  # noqa: F722
    eye_angle: bpy.props.FloatProperty(default=radians(15), min=0, max=0.55, name="Eye angle")  # noqa: F722
    eye_rotate: bpy.props.FloatProperty(default=0.43, min=0, max=0.86, name="Eye rotation")  # noqa: F722
    cheek_ratio: bpy.props.FloatProperty(default=0.5, min=0, max=1, name="cheek position")  # noqa: F722
    cheek_width: bpy.props.FloatProperty(default=0.85, min=0.5, max=1, name="cheek width ratio")  # noqa: F722
    mouth_width_ratio: bpy.props.FloatProperty(default=0.5, min=0.3, max=0.9, name="Mouth width")  # noqa: F722
    # 口角結節
    mouth_corner_nodule: bpy.props.FloatProperty(default=0.1, min=0.01, max=1, name="oris width")  # noqa: F722
    mouth_position_ratio: bpy.props.FloatProperty(default=2 / 3, min=0.3, max=0.7, name="Mouth position")  # noqa: F722
    mouth_flatten: bpy.props.FloatProperty(default=0.1, min=0.0, max=1, name="Mouth flat")  # noqa: F722
    ...
    def make_face(self, context, mesh):
        ...
        neck_point_vert = add_point([-self.head_tall_size / 16, self.neck_tail_y, 0])  # noqa: F841
        ...
        eye_inner_point = eye_point + Matrix.Rotation(-eye_axis, 4, "Y") @ Matrix.Rotation(
            self.eye_rotate, 4, "X"
        ) @ Vector([0, -eye_height, -eye_width])
        eye_outer_point = eye_point + Matrix.Rotation(eye_axis, 4, "Y") @ Matrix.Rotation(
            self.eye_rotate, 4, "X"
        ) @ Vector([0, eye_height, eye_width])
        ...
        make_circle(
            depth_add(eye_point, eye_quad_ru_point[0] - eye_point[0]), eye_iris_size, "Y", 12, 360, 1, 1,
        )
        ...
        nose_end_side_point = depth_add(
            width_add(nose_end_point, max([eye_inner_point[2], self.head_width_size / 6]) * self.nose_width),
            -self.nose_height,
        )
        nose_end_side_vert = add_point(nose_end_side_point)
        nose_end_under_vert = add_point(depth_add(nose_end_point, -self.nose_height))  # noqa: F841
        ...
        mouth_point = Vector(
            [-self.head_depth_size / 2 + self.nose_height * 2 / 3, self.mouth_position_ratio * nose_top_point[1], 0]
        )
        ...
        rotated_height_up = Vector(
            (
                ...
                @ Vector([self.mouth_width_ratio * -0.01 * self.mouth_flatten, self.mouth_width_ratio * 0.01, 0])
            )
        )
        rotated_height_down = Vector(
            (
                Matrix.Rotation(-mouth_rotate_radian, 4, "Z")
                @ Vector([self.mouth_width_ratio * 0.01 * self.mouth_flatten, self.mouth_width_ratio * 0.01 * 1.3, 0])
            )
        )
        ...
        mouth_corner_nodule_point = (
            mouth_outer_point + (mouth_outer_point - mouth_point).normalized() * 0.2 * self.mouth_corner_nodule
        )
        ...
        cheek_point = Vector(
            [-self.head_depth_size / 2, 0, eye_inner_point[2] + (eye_quad_lu_point[2] - eye_inner_point[2]) / 2]
        )
        ...
        tmp_cheek = Matrix.Rotation(eye_axis, 4, "Y") @ Vector(
            [0, 0, (eye_outer_point[2] - eye_inner_point[2] * 2 / 3) * cos(eye_axis) * self.cheek_width]
        )
        ...
        add_mesh([eye_quad_ld_vert, cheek_top_inner_vert, cheek_top_outer_vert, eye_quad_rd_vert])
        add_mesh([cheek_under_inner_vert, cheek_top_inner_vert, cheek_top_outer_vert, cheek_under_outer_vert])
        # eye ring
        add_mesh(
            [arcus_superciliaris_under_vert, arcus_superciliaris_outer_under_vert, eye_quad_ru_vert, eye_quad_lu_vert]
        )
        add_mesh([arcus_superciliaris_under_vert, eye_quad_lu_vert, eye_inner_vert, nose_start_vert])
        ...
        add_mesh([nose_start_vert, cheek_top_inner_vert, cheek_under_inner_vert, nose_end_side_vert])
        add_mesh([nose_end_side_vert, cheek_under_inner_vert, mouth_corner_nodule_vert, mouth_outer_point_vert])
        add_mesh([cheek_under_inner_vert, cheek_under_outer_vert, mouth_corner_nodule_vert])
