...
class VRM_OT_assign_vrm1_expressions_automatically(Operator):
    ...
    def execute(self, context: Context) -> set[str]:
        return assign_vrm1_expressions_automatically(context, self.armature_object_name)
...
class VRM_OT_assign_vrm1_humanoid_human_bones_automatically(Operator):
    ...
    def execute(self, context: Context) -> set[str]:
        ...
        if armature is None:
            return {"CANCELLED"}
        return assign_vrm1_humanoid_human_bones_automatically(context, armature)
...
def draw_bone_prop_search(
    ...
    *,
    ...
    row = VRM_MT_bone_assignment.draw_input_layout(
        layout, human_bone.node, text="", icon=human_bone_specification.icon
    )
    ...
    op = layout_operator(
        row,
        VRM_OT_show_vrm1_bone_assignment_diagnostics,
        text="",
        translate=False,
        icon=icon,
    )
...
class VRM_OT_show_vrm1_bone_assignment_diagnostics(Operator):
    ...
    def draw(self, context: Context) -> None:
        ...
        bpy_bone_name_to_human_bone_specification = {
            bpy_bone_name: HumanBoneSpecifications.get(human_bone_name)
            for human_bone_name, human_bone in human_bone_name_to_human_bone.items()
