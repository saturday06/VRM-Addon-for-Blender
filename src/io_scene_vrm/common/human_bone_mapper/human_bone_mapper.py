...
def create_human_bone_mapping(
    ...
    result = {}
    wanted_required_count = len(
        [True for spec in HumanBoneSpecifications.all_human_bones if spec.requirement]
    )
    if required_count < wanted_required_count:
        structure_based_mapping = create_structure_based_mapping(armature)
        structure_base_mapping_required_count = len(
            [True for spec in structure_based_mapping.values() if spec.requirement]
        )
