...
def assign_humanoid_keyframe(
    ...
    if (
        ...
        if human_bone_name == HumanBoneName.HIPS and translation_keyframes:
            ...
            # TODO: UniVRMは高さを割合で調整しているように見える。
            ...
            if abs(rest_world_translation_z) > 0:
                ...
                parent = bone.parent
                if parent:
                    bone_parent_world_quaternion = parent.matrix.to_quaternion()
                else:
                    bone_parent_world_quaternion = Quaternion()
                world_axis_translation = (
                    bone_parent_world_quaternion.inverted() @ translation
                )
                world_axis_translation.z = world_axis_translation.z * world_height_ratio
                translation = bone_parent_world_quaternion @ world_axis_translation
