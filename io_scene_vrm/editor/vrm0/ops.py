...
class VRM_OT_add_vrm0_blend_shape_group(bpy.types.Operator):
    ...
    def execute(self, _context: bpy.types.Context) -> set[str]:
        ...
        if not isinstance(armature_data, bpy.types.Armature):
            return {"CANCELLED"}
        blend_shape_group = (
            armature_data.vrm_addon_extension.vrm0.blend_shape_master.blend_shape_groups.add()
        )
...
class VRM_OT_add_vrm0_secondary_animation_collider_group(bpy.types.Operator):
    ...
    def execute(self, _context: bpy.types.Context) -> set[str]:
        ...
        if not isinstance(armature_data, bpy.types.Armature):
            return {"CANCELLED"}
        collider_group = (
            armature_data.vrm_addon_extension.vrm0.secondary_animation.collider_groups.add()
        )
