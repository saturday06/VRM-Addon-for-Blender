...
def draw_vrm0_humanoid_layout(
    ...
    row.prop(humanoid, "page", expand=True)
    # row.alignment = "LEFT"
    armature_box.separator()
    if humanoid.page == "page1":
        row = armature_box.row()
        row.alignment = "LEFT"
        row.operator(
    ...
    if humanoid.page == "page2":
        t_pose_box = armature_box.column()
        ...
        armature_box.separator()
        column = armature_box.column()            
        column.label(text="Arm", icon="VIEW_PAN", translate=False)  # TODO: 翻訳
        column.prop(
            humanoid,
            "arm_stretch",
        )
        column.prop(humanoid, "upper_arm_twist")
        column.prop(humanoid, "lower_arm_twist")
        column.separator()
        column.label(text="Leg", icon="MOD_DYNAMICPAINT")
        column.prop(humanoid, "leg_stretch")
        column.prop(humanoid, "upper_leg_twist")
        column.prop(humanoid, "lower_leg_twist")
        column.prop(humanoid, "feet_spacing")
        column.separator()
        column.prop(humanoid, "has_translation_dof")
...
def draw_vrm0_first_person_layout(
    ...
    layout.separator()
    if first_person.page == "page1":
        box = layout.column()
        box.label(text="First Person Bone", icon="BONE_DATA")
        box.label(text="This is recommend to set between eyes")
        box.separator()
        box.prop_search(first_person.first_person_bone, "value", armature.data, "bones", text="Bone")
        box.prop(first_person, "first_person_bone_offset", icon="BONE_DATA", text="Offset")
        layout.separator()
        box = layout.column()
        box.label(text="Mesh Annotations", icon="RESTRICT_RENDER_OFF")
        box.label(text="VRM avatar is recommend to split face and body.")
        box.separator()
...
def draw_vrm0_blend_shape_master_layout(
    ...
    if 0 <= blend_shape_group_index and blend_shape_group_index < len(blend_shape_master.blend_shape_groups):
        ...
        # if 0 <= bind_index and bind_index < len(blend_shape_group.binds):
            ...
            bind_box = box.box().row()
            # row = bind_box.row()
            bind_box.prop_search(
                bind.mesh, "value", blend_data, "meshes", text=""
            )
            ...
            if (
                ...
                bind_box.label(text="No shape key", icon="ERROR")
        ...
        box.label(text="Binds to the property of the materials")
        # row = box.row()
        ...
        for  material_value_index, material_value in enumerate(blend_shape_group.material_values):
        # if 0 <= material_value_index and material_value_index < len(blend_shape_group.material_values):
            ...
            material_value_box = box.box().row()
            column = material_value_box.column()
            ...
            column.prop_search(
                material_value, "material", blend_data, "materials", text="Material"
            )

            column.prop(material_value, "property_name", text="Property")
            # row = material_value_box.row(align=True)
...
class VRM_UL_vrm0_blend_shape_bind(bpy.types.UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            if blend_shape_bind:
                layout.label(text="" if blend_shape_bind.mesh is None else blend_shape_bind.mesh.value,
                             icon="MESH_DATA")                
            else:
                layout.label(text="", translate=False, icon_value=icon)
        elif self.layout_type in {'GRID'}:
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            if material_value_bind:
                layout.label(text="" if material_value_bind.material is None else material_value_bind.material.name, icon="MATERIAL")
...
def draw_vrm0_secondary_animation_layout(
    ...
    bone_groups_box = layout.column()
    row = bone_groups_box.row()
    # row.alignment = "LEFT"
    row.prop(secondary_animation, "page", expand=True)
    bone_groups_box.separator()
    if secondary_animation.page == "page1":
        bone_groups_box.label(text="Bone Groups", icon="GROUP_BONE")
        row = bone_groups_box.row()
        ...
        if 0 <= bone_group_index and bone_group_index < len(secondary_animation.bone_groups):
            ...
            # box.label(text="Center", icon="QUESTION")
            box.label(text="Center can stop the springs on moving.")
            box.label(text="It is the origin of Physics simulation.")
            box.separator()
            box.prop_search(
                bone_group.center,
                "value",
            ...
            box.label(text='Bones of the spring roots')        
            box.separator()
            ...
            box.label(text="Enabled collider groups of the spring")
            box.separator()
            for collider_group_index, collider_group in enumerate(
                bone_group.collider_groups
            ):
                collider_group_row = box.box().row()
    ...
    if secondary_animation.page == "page2":
    #layout.separator()
    # layout.box()
        collider_groups_box = layout.column()
        ...
        if 0 <= collider_group_index and collider_group_index < len(secondary_animation.collider_groups):
            ...
            row = box.row()
            box.prop_search(collider_group.node, "value", armature.data, "bones", text="Parent Bone")
            box.separator()
            box.label(text="Colliders", icon="SPHERE")
            for collider_index, collider in enumerate(collider_group.colliders):
                collider_row = box.box().row()
                collider_row.prop(collider.blender_object, "name", text="", icon="SPHERE")
