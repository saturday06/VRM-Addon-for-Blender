...
__cats_bone_name_to_human_bone_name = {
    ...
    "Thumb0_R": HumanBoneName.RIGHT_THUMB_METACARPAL,
    "Thumb1_R": HumanBoneName.RIGHT_THUMB_PROXIMAL,
    "Thumb2_R": HumanBoneName.RIGHT_THUMB_DISTAL,
    "Thumb0_L": HumanBoneName.LEFT_THUMB_METACARPAL,
    "Thumb1_L": HumanBoneName.LEFT_THUMB_PROXIMAL,
...
def create_human_bone_mapping(
    ...
    for cats_name, human_name in __cats_bone_name_to_human_bone_name.items():
        ...
        if not original_name:
            continue
        specification = HumanBoneSpecifications.get(human_name)
        if not specification:
            logger.error(f"Failed to get the specification of {human_name}")
            continue
